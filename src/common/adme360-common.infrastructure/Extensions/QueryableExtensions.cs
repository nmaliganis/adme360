...
namespace adme360.common.infrastructure.Extensions
{
    ...
    {
        public static QueryResult<T> ApplySort<T>(this QueryResult<T> source, string orderBy,
            Dictionary<string, PropertyMappingValue> mappingDictionary)
        {
            if (source.QueriedItems == null)
            {
            ...
            foreach (var orderByClause in orderByAfterSplit.Reverse())
            {
                ...
                foreach (var destinationProperty in propertyMappingValue.DestinationProperties.Reverse())
                {
                    ...
                    source.QueriedItems = 
                        source.QueriedItems.OrderBy(destinationProperty + (orderDescending ? " descending" : " ascending"));
                }
            }
            return source;
        }
